...
Puppet::Type.type(:package).provide :compressed_eclipse,
:parent => Puppet::Provider::Package do
  ...
  def self.instances_by_name
    Dir.entries("/var/db").find_all { |f|
      f =~ /^\.puppet_compressed_eclipse_installed_/
    }.collect do |f|
      name = f.sub(/^\.puppet_compressed_eclipse_installed_/, '')
  ...
  def self.instances
    instances_by_name.collect do |name|
      new(:name => name, :provider => :compressed_eclipse, :ensure => :installed)
    end
  end
...
private
  ...
  def app_path
    # app_path is nonstandard in this package
